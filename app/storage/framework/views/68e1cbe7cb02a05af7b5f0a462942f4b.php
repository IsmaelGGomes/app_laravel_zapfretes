...
<?php $component->withAttributes([]); ?>
    ...
    <div class="py-12 w-full overflow-x-auto px-2 bg-[#1d232a]">
        <section style="max-width: 100rem" class="flex mx-auto pb-12 px-4 sm:px-6 lg:px-8 ">
            <form action="<?php echo e(config('app.url')); ?>/dashboard" method="get" class="bg-gray-200 rounded-xl py-4  justify-center items-center flex flex-col gap-8 px-4 h-auto mr-10">
...
<script>
    ...
    function deleteContato($id) {
        ...
        const res = fetch(`<?php echo e(config('app.url')); ?>/dashboard/${id}`, options).then((response) => {
            console.log(response.status);
            window.location.reload()
            console.log('sucess');
        }).catch((error) => {
